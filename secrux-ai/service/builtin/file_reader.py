...
router = APIRouter(prefix="/builtin/file-reader", tags=["builtin-file-reader"])
...
def _prepare_response(path: Path, line_range: LineRange, encoding: str) -> FileReadResponse:
...
@router.post("/tools/file.read.full:invoke", response_model=FileReadResponse)
def read_full(request: FileReadRequest) -> FileReadResponse:
    safe_path = _safe_path(request.path)
    return _prepare_response(safe_path, request.line_range or LineRange(), request.encoding)


@router.post("/tools/file.read.range:invoke", response_model=FileReadResponse)
def read_range(request: FileReadRequest) -> FileReadResponse:
    if request.line_range is None:
        raise HTTPException(status_code=400, detail="lineRange must be provided for range reads")
    safe_path = _safe_path(request.path)
    return _prepare_response(safe_path, request.line_range, request.encoding)
