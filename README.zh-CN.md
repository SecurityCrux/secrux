...
- 启动带插件的 IDE：`cd secrux-intellij-plugin && ../gradle-8.10.2/bin/gradle runIde`
- 构建插件 ZIP：`cd secrux-intellij-plugin && ../gradle-8.10.2/bin/gradle buildPlugin`
...
- Sink 列表：支持内置列表或项目自定义 JSON 文件（参考 `secrux-intellij-plugin/docs/sink-catalog.example.json`）
